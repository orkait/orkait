import { clamp, easeInOutCubic, easeOutCubic, mix, phase } from "@/hooks/use-lenis-timeline";
...
export function getServicesTimelineStyles(progress: number): ServicesTimelineStyles {
    const mediaP = easeOutCubic(phase(progress, 0, 0.24));
    const tagsP = easeOutCubic(phase(progress, 0.24, 0.42));
    const panelP = easeOutCubic(phase(progress, 0.42, 0.56));
    ...
    return {
        ...
        mediaOpacity: mix(0.12, 1, mediaP) * mix(1, 0.5, outroP),

        tagsX: mix(30, 0, tagsP),
        tagsY: mix(8, 0, tagsP),
        tagsOpacity: mix(0.1, 1, tagsP) * mix(1, 0.62, outroP),

        panelY: mix(38, 0, panelP) + mix(0, -10, outroP),
        panelOpacity: mix(0.14, 1, panelP) * mix(1, 0.46, outroP),
    };
}
...
export function getServiceItemStyle(progress: number, index: number, activeIndex: number) {
    const stepsProgress = phase(progress, SERVICE_STEP_START, SERVICE_STEP_END);
    ...
    return {
        translateY: isFuture ? mix(18, 0, eased) : mix(10, 0, eased),
        scale: isFuture ? mix(0.94, 0.97, eased) : mix(0.96, 1, eased),
        opacity: isFuture ? mix(0.26, 0.55, eased) : mix(0.5, 1, eased),
