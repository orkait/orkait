...
export function HeroScene() {
    ...
    const updateTimeline = useCallback(({ progress, effectIndex }: LenisTimelineFrame) => {
        const styles = getHeroTimelineStyles(progress);
        ...
        if (introRef.current) {
            introRef.current.style.transform = `translate3d(0, ${styles.introY.toFixed(2)}px, 0)`;
            introRef.current.style.opacity = styles.introOpacity.toFixed(3);
        }

        if (titleGroupRef.current) {
            titleGroupRef.current.style.transform =
                `translate3d(0, ${styles.heroLift.toFixed(2)}px, 0) ` +
        ...
        if (logoWrapRef.current) {
            logoWrapRef.current.style.transform =
                `translate3d(0, ${styles.logoY.toFixed(2)}px, 0) ` +
        ...
        if (mainWordRef.current) {
            mainWordRef.current.style.transform =
                `translate3d(0, ${styles.mainWordY.toFixed(2)}px, 0) ` +
